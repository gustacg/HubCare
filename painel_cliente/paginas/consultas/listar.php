...
if($status == "%%"){
	...
	$query = $pdo->query("SELECT * FROM $tabela where data >= '$dataInicial' and data <= '$dataFinal' and status != 'Finalizado' and paciente = '$paciente' ORDER BY data asc, hora asc");

}else{

	$query = $pdo->query("SELECT * FROM $tabela where data >= '$dataInicial' and data <= '$dataFinal' and status LIKE '$status' and paciente = '$paciente' ORDER BY data asc, hora asc");
...
<td class="">{$nome_medico}</td>
<td class="">{$nome_convenio}</td>
