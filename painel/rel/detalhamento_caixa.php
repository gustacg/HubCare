					...
					<?php 
						...
						if(@count($res2) > 0){
							for($i2=0; $i2 < @count($res2); $i2++){
								...
								$query3 = $pdo->query("SELECT * FROM receber where usuario_pgto = '$operador' and data_pgto >= '$data_abertura' and data_pgto <= '$data_fechamento_consulta' and saida = '$id_pgto' and caixa = '$id'");
...
<td style="width:22%"><?php echo $descricao ?></td>

<td style="width:15%;"><?php echo $referencia ?> </td>

<td style="width:9%; color:green">R$ <?php echo $valorF ?></td>
					...
					<?php 
						...
						if(@count($res2) > 0){
							for($i2=0; $i2 < @count($res2); $i2++){
								...
								//total de entradas por essa forma de pgto	
								$total_recebido_pgto = 0;
								$query3 = $pdo->query("SELECT * FROM pagar where usuario_pgto = '$operador' and data_pgto >= '$data_abertura' and data_pgto <= '$data_fechamento_consulta' and saida = '$id_pgto' and caixa = '$id'");
								$res3 = $query3->fetchAll(PDO::FETCH_ASSOC);
								if(@count($res3) > 0){
									for($i3=0; $i3 < @count($res3); $i3++){
										$valor_rec_pgto = $res3[$i3]['valor'];
		...
		<table id="cabecalhotabela" style="width: 100%; table-layout: fixed; font-size:9px; text-transform: uppercase;">
			...
			<thead>
				...
				<tr id="cabeca" style="margin-left: 0px; background-color:#CCC; font-weight: bold">
					...
					<td style="width:18%">FUNCIONÁRIO</td>
...
<td style="width:9%; color:red">R$ <?php echo $valorF ?></td>

<td style="width:9%"><?php echo $data_pgtoF ?></td>

<td style="width:9%; "><?php echo $horaF ?></td>

<td style="width:18%; "><?php echo $nome_pessoa ?></td>
