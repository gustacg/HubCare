...
$query2 = $pdo->query("SELECT * from valor_parcial where data >= '$dataInicial' and data <= '$dataFinal' and tipo = '$tipo_residuo' order by id desc ");
...
if(@count($res2) > 0){
	...
	for($i2=0; $i2 < @count($res2); $i2++){	
			...
			$id_conta = $res2[$i2]['id_conta'];

			$usuario_res = $res2[$i2]['usuario'];

			

			$valorF = number_format($valor, 2, ',', '.');

			$data_pgtoF = implode('/', array_reverse(@explode('-', $data)));

			$data_lancF = $data_pgtoF;
			...
			$query3 = $pdo->query("SELECT * from $filtro_tipo where id = '$id_conta' ");

			$res3 = $query3->fetchAll(PDO::FETCH_ASSOC);

			$descricao = '(Resíduo) '.@$res3[0]['descricao'];

			$data_venc = @$res3[0]['data_venc'];

			$data_vencF = implode('/', array_reverse(@explode('-', $data_venc)));

			$referencia = @$res3[0]['referencia'];



			$query3 = $pdo->query("SELECT * from usuarios where id = '$usuario_res' ");

			$res3 = $query3->fetchAll(PDO::FETCH_ASSOC);

			$nome_usu_pgto = @$res3[0]['nome'];
